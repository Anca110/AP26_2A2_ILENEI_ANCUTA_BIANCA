...
public class MovieSolver {
    ...
    public List<MovieList> solve(List<Movie> movies) {
        ...
        for (int k=1; k <=movies.size(); k++) {
            List<MovieList> solution = solveKLists(movies, related, k);
    ...
    private List<MovieList> solveKLists(List<Movie> movies, boolean[][] related, int k) {
        int n = movies.size();
        ...
        int minSize = n / k;
        int maxSize = (n % k == 0) ? minSize : minSize + 1;
        ...
        for (int listIndex = 0; listIndex < k; listIndex++){
            ...
            IntVar count = model.intVar("count" + listIndex, minSize, maxSize);
            model.sum(inList, "=", count).post();
        }
